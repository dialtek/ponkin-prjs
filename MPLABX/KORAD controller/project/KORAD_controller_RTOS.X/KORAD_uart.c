...
volatile QueueHandle_t KORADrxBytes;
...
void Uart2Init()
{
   // RX interrupt UART2 settings 
    ...
    KORADrxBytes = xQueueCreate(16, sizeof(unsigned char));
 }
...
signed short Uart2GetChar(unsigned char *pcRxedChar, TickType_t xBlockTime )
{
	...
	if( xQueueReceive(KORADrxBytes, pcRxedChar, xBlockTime ) )
	{
		return pdTRUE;
	}
	else
...
void _ISR_PSV _U2RXInterrupt(void)      //interupt UART 1 RX IP702C
{
      ...
      xQueueSendFromISR(KORADrxBytes, (void*)&RxByte, 0);
}

UBaseType_t Uart2GetQcnt(void)
{
   return uxQueueMessagesWaiting(KORADrxBytes); 
}

void Uart2ResetQueue(void)
{ 
    xQueueReset(KORADrxBytes);
}
