...
unsigned int rd_status = 0, rd_voltage = 0, rd_current = 0,
...
unsigned int BaseCnt = 0, ErrCnt = 0, IdleCnt = 0;
...
void Timer9_init(unsigned long baudrate) {
    ...
    switch (baudrate) {
        ...
        case 115200: PR9 = 8400;
            break; // 1 byte - 70 us, 2800 * 3 = 210 us
...
void U1_send_byte(unsigned char Ch) { //  U1 send byte
...
void uart2_send_hex(unsigned char Ch) { // RS-232 send byte
...
unsigned char char2num(unsigned char ch) { // converts char symbol into number
    return ch - 0x30;
}

unsigned char num2char(unsigned char ch) { // converts number into char symbol
    return ch + 0x30;
}
...
void PSP405_set_current(unsigned int current) { // current in mA
    // get chars from number
...
void PSP405_rx_parse(unsigned char parse_state) {
    switch (parse_state) {
        ...
        case 1:
            ...
            if (uart2_rx_buf[0] == 'V' && (uart2_rx_buf[37] == 0x0D || uart2_rx_buf[38] == 0x0D)) { // ok, got nice answer
            ...
            if (uart2_rx_buf[6] == 'A' && (uart2_rx_buf[37] == 0x0D || uart2_rx_buf[38] == 0x0D)) { // ok, got nice answer
            ...
            if (uart2_rx_buf[18] == 'U' && (uart2_rx_buf[37] == 0x0D || uart2_rx_buf[38] == 0x0D)) { // ok, got nice answer
                rd_voltage_lim = char2num(uart2_rx_buf[19]) * 10 + char2num(uart2_rx_buf[20]);
            }
            //------- checking src answer to be Ii.ii<cr>
            if (uart2_rx_buf[21] == 'I' && (uart2_rx_buf[37] == 0x0D || uart2_rx_buf[38] == 0x0D)) { // ok, got nice answer
            ...
            if (uart2_rx_buf[26] == 'P' && (uart2_rx_buf[37] == 0x0D || uart2_rx_buf[38] == 0x0D)) { // ok, got nice answer
            ...
            if (uart2_rx_buf[30] == 'F' && (uart2_rx_buf[37] == 0x0D || uart2_rx_buf[38] == 0x0D)) { // ok, got nice answer
...
void PSP405_state_restore(void) {
    ...
    PSP405_set_output(0);
    holding_register[12] = 0; // clear out state
    __delay_ms(400);
...
void Read_PSP_srs(void) {
    if (curr_cmd == NO_CMD_RUNNING) // reading src if no cmds executed at the moment
    {
        ...
        } else // if source no respond 
        {
            BaseCnt++;
            __delay_ms(1);
            if (BaseCnt > READ_TIMEOUT * 2) {

                BaseCnt = 0;
                ErrCnt++;

                if ((ErrCnt > 2) && (curr_cmd != SRC_CMD)) {
                    PSP405_rx_parse(0);
                    data_ready = 0;
                    ErrCnt = 0;
                    curr_cmd = IDLE;
...
int main(void) {
    ...
    curr_cmd = IDLE; // able to read src
