...
10.составить регистровую карту устройства в фукции modbus_refresh() :
      A) при чтении регистров, выполн€етс€ код if(cmd_type == MODBUS_RHR_CMD)
      B) при записи регистров, выполн€етс€ код if(cmd_type == MODBUS_WSR_CMD)

11. при необходимости использовани€ своих функции внутри modbus_refresh(), следует создать их прототипы в поле PROTOTYPES  
*/

/*############################################################################*/ 
/*                   ЊЭО€, їЂЮ?Н€?ЮЯ? ЊЭОєЂЭђЉУ?О?Ю !!!                       */ 
...
/* HARDWARE INFO */
  ...
  unsigned char firmware_ver  = 15;                 // верси€ прошивки текущего устройства
