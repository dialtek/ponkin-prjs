...
//  }
  ...
  void modbus_wsr_answer() { 
    ...
    // расчет CRC
    crc_buf[0] = (unsigned char) dev_id;//(rx_buf[0] == dev_id) ? dev_id : com_dev_id;
    ...
    TX_EN;
    uart_send_byte((unsigned char)dev_id);//(rx_buf[0] == dev_id) ? dev_id : com_dev_id); // ID устройства
  ...
  void modbus_rhr_answer() {
    ...
    /// ответ на команды чтения регистров
  	  ...
  	  addr_buf_2 = addr_buf_1;             // сохр. адрес в двух переменных	  
          // расчет CRC
          crc_buf[0] = dev_id;//(rx_buf[0] == dev_id) ? dev_id : com_dev_id;
  ...
  if(rx_flag)			// state 1, rx timer overflows
  {
   ...
   {   
    if(rx_buf[0] == dev_id)//(rx_buf[0] == dev_id) || rx_buf[0] == com_dev_id)     // state 3, ID check 
    {// ID OK
