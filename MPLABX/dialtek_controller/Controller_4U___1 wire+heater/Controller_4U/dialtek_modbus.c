/*                D I A L T E K    M O D B U S   R T U   v 2.1                */

/* »Ќ—“–”ћ≈Ќ“џ ѕќЋ№«ќ¬ј“≈Ћя */

/* README */
/*
ƒл€ работы библиотеки MODBUS RTU необходимо:

1. настроить прерывание UART RX по приему одного байта 

2. вставить в обработчик прерывани€ UART RX функцию modbus_uart_byte, например:
      modbus_uart_byte(MDR_UART1->DR); или modbus_uart_byte(U1RXREG);

3. создать функцию отправки одного байта UART TX следующего вида:
      void uart_send_hex(unsigned char hex_data){ }

4. поместить функцию modbus_rx_sm() в обработчик прерывани€ по таймеру (“ >= 1 мс) или цикл while(1)

5. поместить функцию modbus_init() в main()

6. поместить функцию modbus_poll() в цикл while(1) (“ >= 1 мс)

7. настроить порты дл€ управлени€ разрешением работы приемника (если необходимо) и передатчика в поле RS485 EN/DIS DEFINE

8. настроить аттрибуты устройства в поле HARDWARE INFO 

9. заполнить функцию modbus_int_mode() кодом с разрешением и запретом прерываний UART RX

10.составить регистровую карту устройства в фукции modbus_refresh() :
      ј) при чтении регистров, выполн€етс€ код if(cmd_type == MODBUS_RHR_CMD)
      Ѕ) при записи регистров, выполн€етс€ код if(cmd_type == MODBUS_WSR_CMD)

11. при необходимости использовани€ своих функции внутри modbus_refresh(), следует создать их прототипы в поле ѕ–ќ“ќ“»ѕџ  
*/

/*############################################################################*/ 
/*                    ѕќЋя »«ћ≈Ќя≈ћџ≈ ѕќЋ№«ќ¬ј“≈Ћ≈ћ !!!                       */ 

/* RS485 EN/DIS DEFINE */

#define TX_EN  LATEbits.LATE8=1
#define TX_DIS LATEbits.LATE8=0

#define heater_on  LATCbits.LATC0 = 1
#define heater_off LATCbits.LATC0 = 0

/* HARDWARE INFO */
  unsigned char dev_id        = DEFAULT_DEV_ID;     // MODBUS ID устройства                                      <<<<<<<<<<======== ID
  unsigned char firmware_ver  = 11;                 // верси€ прошивки текущего устройства
  unsigned char device_family = 200;                 // код семейства устройств

/* ѕ–ќ“ќ“»ѕџ */
  void SPI_WRITE_WORD (unsigned int DATA);
 unsigned int SPI_READ_WORD (unsigned int ADDR);
 void CPLD_SPI_WR (unsigned int hold_reg_data, unsigned int  hold_reg_addr);
 unsigned int CPLD_SPI_RD (unsigned int hold_reg_addr); 
 
 /* USER FUNCS */
/*--------------------------------------------------------------------------- */  
// 1- wire 
#define ID_lenght	8 // кол-во байт в ID датчика
#define MAX_sens_qty 10   // макс. число датчиков 

unsigned char ID_buf [8]     = {0,0,0,0,0,0,0,0};     // временный массив ID
unsigned char index_buf [10] = {255,255,255,255,255,255,255,255,255,255}; // массив с зан§тыми ID
unsigned char add_count;       // счетчик попыток добавлени§
unsigned char eq_cnt = 0;      // счетчик совпадений массива ID  
unsigned char index_exist = 0; // флаг наличи§ индекса среди уже записанных

unsigned char Sens_ID[MAX_sens_qty][ID_lenght]; // ID датчиков в двумерном виде  
unsigned char sensors_qty  = 0;			// кол-во датчиков на шине 1-wire
unsigned char sensor_index;       // индекс датчика дл§ сохр. его id

unsigned long hysteresis = 3000;
unsigned long orig_temp  = 38000;


  // запись ID нового датчика во временный массив
  void get_sensor_ID()
  {
     if (OneWire_reset())
     { 
        OneWire_send(Read_ROM);
        for (unsigned char i = 0; i < ID_lenght; i++)
            ID_buf[i] = OneWire_read(); // сохранение ID датчика во временный буфер 
     }
  }
  
  // поиск ID нового датчика среди уже сохраненных
  unsigned char match_sensor_ID()
  {
     unsigned char sensor_exist = 0;
      
     // сравнение ID нового датчика с теми, что были добавлены ранее
     for(unsigned char i = 0; i < sensors_qty; i++)
     {
        for(unsigned char j = 0; j < ID_lenght; j++)
        {
            if(Sens_ID[i][j] == ID_buf[j])
                eq_cnt++;          
        }
        
        if(eq_cnt < ID_lenght) eq_cnt = 0; // сброс счетчика перед новым поиском
     }
      
     if(eq_cnt >= ID_lenght) 
         sensor_exist = 1;
      
      return sensor_exist;
  }

  // сброс конфигурации датчиков
  void reset_cfg()
  {
      // очитска массива с ID датчиков       
      for(unsigned char i = 0; i < MAX_sens_qty; i++)
      {
         index_buf[i] = 255;  // массив с зан§тыми ID
                 
         for(unsigned char j = 0; j < ID_lenght; j++) 
             Sens_ID[i][j] = 0;
      }
         
      // очитска регистров, содержащих ID датчиков  
      for(unsigned char i = 10; i < 110; i++)
      {
          input_register[i] = 0;
          holding_register[i-10] = 0;
      }
      
      sensors_qty = 0;                     // сброс числа датчиков    
      add_count = 0;
      index_exist = 0;
  }
  /*--------------------------------------------------------------------------- */  
  
  
  void modbus_int_mode (unsigned char mode)  
  {      // управление прерыванием UART RX
  
    if(mode == 1) // разрешение прерываний RX UART MODBUS
    {
      IEC0bits.U1RXIE = 1; // en UART1 RX interrupt   
    }
    else         // запрет прерываний RX UART MODBUS
    {
      IEC0bits.U1RXIE = 0; // dis UART1 RX interrupt   
    }
  }
  
  void modbus_refresh(unsigned char cmd_type) // работа с регистрами
  {     
    
   /// „тение R/W регистров, TODO - обновление переменных перед отправкой мастеру    
    
   if(cmd_type == MODBUS_RHR_CMD)
   {
     
	 MODBUS_RX_LED = 1;
       
       
    long t = 0;
    unsigned char cnt = 0;

    if(sensors_qty > 0)
    {
    OneWire_all_rd_t(); // †оманда измерени§ У дл§ всех датчиков
                        // задержка измер. У 760 мс внутри функции

        for(unsigned char i = 0; i < sensors_qty; i++)// Эпрос Уemper с датчиков поочередно
        {
          OneWire_match_ID(Sens_ID[i]);
          t = ds18b20_i_rd_t();  //присвоение  Temper значени§ температуры
       
          holding_register[cnt]   = (unsigned int)  (t >> 16) ;
          cnt++;
          holding_register[cnt] = (unsigned int)  (t & 0x0000ffff);
          cnt++;
          
        }
        
        if ( t >= orig_temp)        
            heater_off;
        
        if (t < (orig_temp - hysteresis))
            heater_on;
          
        holding_register[30] = sensor_index;
        
    }
    
    holding_register[6] = orig_temp;
    holding_register[7] = hysteresis;
    
    holding_register[41] = CPLD_SPI_RD(1);
    holding_register[42] = CPLD_SPI_RD(2);
    holding_register[43] = CPLD_SPI_RD(3);
    holding_register[44] = CPLD_SPI_RD(4);
    
    delay_ms(10);
    MODBUS_RX_LED = 0;
    
   }         
   //-------------------------------------------------------------------------//
   /// „тение Read-only регистров, TODO - обновление переменных перед отправкой мастеру   
   if(cmd_type == MODBUS_RIR_CMD)
   {   
       MODBUS_RX_LED = 1;
        
       input_register[0] = (unsigned int)dev_id;       
       input_register[1] = (unsigned int)device_family;
       input_register[2] = (unsigned int)firmware_ver;
       
       input_register[9] = (unsigned int)sensors_qty;
       
       delay_ms(10);
       MODBUS_RX_LED = 0;
   }         
   //-------------------------------------------------------------------------//
  
   /// «јѕ»—№, TODO - получение новых значений от мастера
   //  addr_buf_1 = адрес регистра, в который идет запись (1000 = 0, 1001 = 1 и тд)
      
   if(cmd_type == MODBUS_WSR_CMD) 
   {  // анализ регистра записи 
      switch(addr_buf_1) 
      {  
        case 1: // reg 4 - set output

        break;
        //---------
        case 6: // reg 6 - needed temp
            
            orig_temp = holding_register[6];

        break;
        //---------
        case 7: // reg 7 - hysteresis
            hysteresis = holding_register[7];
        break;
        //---------
        case 30: // set sensor number
          
          // добавл§ем новый датчик, если его индекс в диапазоне 0 - MAX_sens_qty-1
          if(holding_register[30] <= MAX_sens_qty -1)
          {
            // смотрим, бы ли ранее записан этот индекс датчика
            for(unsigned char i = 0; i < MAX_sens_qty; i++)
            {
                if(index_buf[i] == holding_register[30])
                    index_exist = 1; // да, взводим флаг
            }
            
            // если это новый датчик..
            if(!index_exist)
            {
               index_buf[add_count] = sensor_index; // сохр. его индекс в буфер
               add_count++; // инкремент числа операций добавлени§
            }
          }
          
          else holding_register[30] = MAX_sens_qty - 1;
          sensor_index = holding_register[30];
          
          if(add_count > 10) add_count = 0;
            
        break;
        //---------
        case 31: // добавление нового датчика
        
        eq_cnt = 0;  // сброс счетчика совпадений байт ID
        
        //get 8 ROM bytes - считывание ID датчка во временный буфер
        get_sensor_ID();  
        
        // сброс операции добавлени§ датчика, если он добавлен ранее
        if(match_sensor_ID() == 1) break;            
        // если это новый датчик - добавл§ем его
        else
        {
            for (unsigned char i = 0; i < ID_lenght; i++) 
            {
              input_register[(10+10*sensor_index) + i] = ID_buf[i];  
              Sens_ID[sensor_index][i] = input_register[(10+10*sensor_index) + i];
            } 
            
            if(index_exist)
              index_exist = 0; // сброс флага наличи§ индекса среди уже записанных
            else 
            {
                sensors_qty++; // инкремент числа датчиков, если это новый
                index_exist = 0;
            }
            sensor_index = 255;
           
          // 1st sensor ID: 10 - 17 input regs
          // 2nd sensor ID: 20 - 27 input regs
          // etc.
        }
          
        break;
         //---------
        case 32:  
        /* очистка массива с ID и возвражение прибора к исходному состо§нию
          без датчиков */
            
            reset_cfg();
        break;
        //---
        case 71: CPLD_SPI_WR (holding_register[71],1);    
        break;
        
        case 72: CPLD_SPI_WR (holding_register[72],2);    
        break;
        
        case 73: CPLD_SPI_WR (holding_register[73],3);    
        break;
        
        case 74: CPLD_SPI_WR (holding_register[74],4);         
        break;
        
        default: break;  
      }
      
      //eeprom_wr_regs(); // save registers state
   }
   
      
   
  }

/*############################################################################*/  
  
/*                         M O D B U S   E N D                                */




