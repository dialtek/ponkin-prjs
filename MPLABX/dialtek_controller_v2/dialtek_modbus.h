...
unsigned int starting_address, addr_buf_2;
...
#define DEFAULT_DEV_ID (unsigned char) 200
#define MODBUS_RX_LED LATCbits.LATC1 // LED_RX_485
#define MODBUS_TX_LED LATCbits.LATC2 // LED_TX_485
...
#include "dialtek_modbus.c"
  ...
  unsigned char is_reg (unsigned int reg_addr){  
   ...
   unsigned char rd_status = 0;
      ...
      for(unsigned int t = starting_address; t <= regs2read; t++)
      { 
        if (t == reg_addr) 
	{
	 rd_status = 1;  
...
return rch;
  ...
  void modbus_rhr_answer() {
    ...
    /// ответ на команды чтения регистров
  	  addr_buf_2 = starting_address;             // сохр. адрес без смещения в двух переменных	  
          // расчет CRC
          crc_buf[0] = dev_id;
          ...
          { // заполнение буфера CRC для расчета
            crc_buf[cnt++] = (unsigned char)(holding_register[starting_address] >> 8);
            crc_buf[cnt++] = (unsigned char)(holding_register[starting_address] & 0x00ff);
            ++starting_address;  
          }
            // расчет CRC  
  ...
  void modbus_rir_answer() {
    ...
    /// ответ на команды чтения input регистров
  	  ...
  	  addr_buf_2 = starting_address;             // сохр. адрес без смещения в двух переменных	  
          ...
          for(unsigned char i = 0; i < regs2read; i++)
          { // заполнение буфера CRC для расчета
            crc_buf[cnt++] = (unsigned char)(input_register[starting_address] >> 8);
            crc_buf[cnt++] = (unsigned char)(input_register[starting_address] & 0x00ff);
            ++starting_address;  
          }
            // расчет CRC  
          CRC16 = modbus_CRC16(crc_buf,(regs2read*2)+3);
  ...
  void modbus_poll()  {
    ...
    modbus_rx_sm();	// analyze rx bytes
		 
    /// update modbus regs and vars, send answer to master
    starting_address = modbus_reg_addr;
