...
/* USER FUNCS */
  ...
  void modbus_refresh(unsigned char cmd_type){      // работа с регистрами
   ...
   {   
       holding_register[0] = servo_step_qty;
       holding_register[1] = servo_step_period;
       holding_register[2] = servo_dir;
       holding_register[3] = servo_busy;    
   }         
   //-------------------------------------------------------------------------//
   ...
   {  // анализ регистра записи 
      switch(addr_buf_1) {  
        case 0: // reg 0 - steps
           ...
           servo_step_qty  = holding_register[0];
        break;
        //---
        case 1: // reg 1 - period
            if(holding_register[1] < 3) holding_register[1] = 3;
            servo_step_period = holding_register[1];
        break;
        //---
        case 2: // reg 2 - direction
            if(holding_register[2] < 1) holding_register[2] = 1;
           servo_dir  = holding_register[2];
        break;
        //---
