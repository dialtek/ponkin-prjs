...
void I2C_init(unsigned int FCLOCK)
{
    ...
    I2C2CONbits.I2CSIDL = 0;    // Continues the module operation in the Idle mode
    I2C2CONbits.SCLREL = 1;     // Releases the SCLx clock
    I2C2CONbits.A10M = 0;       // 0 - 7-bit slave address, 1 - 10 bit
    I2C2CONbits.SMEN = 0;       // Disables the SMBus input thresholds
    I2C2CONbits.ACKDT = 0;      // Sends an ACK during an Acknowledge
    I2C2CONbits.RCEN = 1;       // Enables Receive mode for I2C   
    I2C2CONbits.IPMIEN = 0;     // should not be set when operating as a master
}
...
// I2C EEprom 
 ...
 void ROM_wr_byte (unsigned short addr, unsigned char dataByte)
 {
    ...
    I2C_write (dataByte);
    I2C_stop ();
    __delay_ms (15); // t записи в rom - 10 мс
...
//    crc_wr_index++;
  ...
  unsigned char ROM_rd_byte (unsigned short addr)
 {
    ...
    I2C_start (EEprom_addr|1);// чтение
