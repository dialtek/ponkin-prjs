...
/* HARDWARE INFO */
  ...
  unsigned char device_family = 20;                 // код семейства устройств
...
/* USER FUNCS */
  ...
  void modbus_refresh(unsigned char cmd_type){      // работа с регистрами
   ...
   {   
       holding_register[0] = motor_step_qty;
       holding_register[1] = motor_step_period;
       holding_register[2] = motor_dir;
       holding_register[3] = motor_busy;    
   }         
   //-------------------------------------------------------------------------//
...
//       input_register[2] = (unsigned int)firmware_ver;
   ...
   {  // анализ регистра записи 
      switch(addr_buf_1) {  
        case 0: // reg 0 - steps
            if(holding_register[0] < 1) holding_register[0] = 1;
           motor_step_qty  = holding_register[0];
        break;
        //---
        case 1: // reg 1 - period
            if(holding_register[1] < 10) holding_register[1] = 10;
            motor_step_period = holding_register[1];
        break;
        //---
        case 2: // reg 2 - direction
            if(holding_register[2] < 1) holding_register[2] = 1;
           motor_dir  = holding_register[2];
        break;
        //---
        case 10: // reg 10 - steps
           motor_start = 1;
        break;
        //---
          default: break;
