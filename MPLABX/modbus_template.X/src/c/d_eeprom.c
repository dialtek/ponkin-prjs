...
void eeprom_wr_page(unsigned char reg_type, unsigned int address)
{
  ...
  for(unsigned int i = 0; i < 32; i++) // max bytes to write - 64
  {
    ...
    else if(reg_type == INPUT_REGS)
        input_reg_read((i + (address-INPUT_REGS_ADDR)/2),&RegVal);
    
    SPI3_write_byte((unsigned char)(RegVal >> 8));     // data msb    
    SPI3_write_byte((unsigned char)(RegVal & 0x00ff)); // data lsb 
 }
  
  delay_us(5);
  CS3_HIGH;
  
  delay_ms(10);  // Internal Write Cycle Time × 5 ms
}
