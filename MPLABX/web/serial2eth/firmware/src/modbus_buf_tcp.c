...
static int16_t wMaxGet = 0, wMaxPut = 0, wCurrentChunk = 0, j = 0;

unsigned int TCP_RxCnt(void)
{
  return (unsigned int)wMaxGet;
}
...
int get_rx_buf_ptr(void)
{
  return modbus_rx_buf_ptr;
}

unsigned char* get_rx_buf(void)
{
  return (void*)&modbus_rx_buf;
}
...
void MODBUS_BUF_TCP_Tasks (void)
{
    ...
    switch(tcpAppData.state)
    {
        ...
        case MODBUS_BUF_TCPIP_SERVING_CONNECTION:
        {
            ...
            else  // clear rx buf if no data received
              for(j = 0; j < MODBUS_RX_BUF_LENGTH; j++)
                modbus_rx_buf[j] = 0;
            ...
            if (modbus_tx_buf_ptr > 0) // >=
            {
                ...
                for(j = 0; j < MODBUS_TX_BUF_LENGTH; j++)
                  modbus_tx_buf[j] = 0;
            }
