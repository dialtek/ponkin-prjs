...
#define ADC_Vref 3.3		 // измеренное вольтметром напряжение питания МК
  ...
  U8 need2update = 0;
  U8 send_status = 0;  // флаг запроса на выдачу статуса по UART	
...
//функция обработки прерывания irq Timer 3 - СТРОБЫ ВНЕШНЕЙ СИНХРОНИЗАЦИИ
{
    ...
    {   
        ...
        IVC_START_INT;                      // начало интегрирования
        delay_us(int_time); 
        //delay_us(int_time);                   // ждем установленное время интегрирования
        //MDR_PORTC->RXTX = ~ MDR_PORTC->RXTX;  // конец интегрирования DDC, изменение лог. состояния строба 
        IVC_STOP_INT;                         // конец интегрирования IVC
        need2update = 1;
        SYNC_LED_ON;   
     } 
    
//     else//if(int_flag & (1<<1)) //обработка прерывания Событие совпадения CNT с ARR
//    {   
//        MDR_PORTC->RXTX = ~ MDR_PORTC->RXTX;            
//        delay_us(50);
//        MDR_PORTC->RXTX = ~ MDR_PORTC->RXTX;   
...
{
  /// прерывание возникает при поступлении 8 байт в буфер FIFO приемника UART1

         // 12 бит. буфер FIFO, глубина 16 слов - 7…0 Принятые данные 
         U16 Rx_data[16];
         U8 rd_byte_cntr = 0;
         while(FIFO_has_byte) // пока буфер не пуст
         {
          Rx_data[rd_byte_cntr] = MDR_UART1->DR; // чтение содержимого буфера
         ...
         {
              case 'S':         // команда проверки связи с прибором 
                send_status = 1;// запрос получен, выставлен флаг отсылки статуса на ПК
              break;
              //--
              ...
              case 'M':        // команда "Выбор режима измер", 0 - пост, 1 - имп
                ...
                {
                  MDR_TIMER3->CNTRL &= ~1;      // стоп Т3
                  MDR_TIMER1->CNTRL &= ~1;      // стоп счетчика времени м/у имп.
                  trig_timeout = 0;
                }
                else
                {
                  MDR_TIMER3->CNTRL |= 1;       // ловим фронт этим таймером
                  MDR_TIMER1->CNTRL |= 1;       // старт счетчика времени м/у имп.
                  trig_timeout = 0;
                }
              break; 
              //--
         ...
         MDR_UART1->RSR_ECR = 0x00000000;  // сброс статуса заполнения FIFO
                                           // cброс прерывания от приемника UARTRXINTR
         MDR_UART1->ICR  = 1<<4 | 1 << 10; // cброс прерывания по переполнению буфера UARTOEINTR 
}
 
__irq void Timer1_IRQHandler(void) // обработчик прерывания от Таймера 1
{   
      ...
      void MCU_init (void) 
    {
      ...
      timer1_init();		// отсчет 15 сек для подтверждения связи с ПК в импульсном режиме
 ...
 int main()
{
 ...
 while(1)
  { 
     ...
     if(send_status)          // отсылка статуса прибора
    {
       send_status = 0;      // сброс флага отсылки статуса
...
/// режим непрерывных измерений  
      ...
      { 
        SYNC_LED_ON;                  // вкл. светодиод
        need2send = 0;                // сброс флага запроса
        //-- DDC --
        for(U8 i = 0; i < 40; i++)
        {
          MDR_PORTC->RXTX = ~ MDR_PORTC->RXTX;      // изменение лог. состояния строба интегрирования        
          delay_us(int_time);
          MDR_PORTC->RXTX = ~ MDR_PORTC->RXTX;      // изменение лог. состояния строба интегрирования   
          delay_us(int_time);
          if(i == 37) 
       ...
       // чтение отсчетов АЦП 
        ...
        delay_ms(50); // держим светодиод включенным
        SYNC_LED_OFF; // гасим светодиод
      }
      break;
//------------------------------------------------------------------------------ 
...
/// импульсный режим 
    ...
    if(trig_timeout > 14)
    { // если не дождались импульса..подтверждение соединеия
      
      trig_timeout = 0;    // сброс переменной ожидания импульса
      Uart_send_text("Z"); // отправка символа подтверждения связи
      Uart_CR_LF_send();
    }  
      
    if(need2update) // анализ флага обновления данных
     { 
       // сигнал проинтегрирован, ждем готовности преобразования
       ...
       full_current -= ADC_noise;            // убираем смещение интегратора
       full_current *= OpAmpK;               // приводим к динамич. диапазону АЦП МК, см. define
       need2update = 0;                      // сброс флага обновления данных
...
// выдвигается сначала последний вход АЦП, потом предпоследний и тд   
	 ...
	 Uart_CR_LF_send();         
         ...
         //delay_ms(20);                      // держим светодиод включенным
         SYNC_LED_OFF;                      // гасим светодиод
         trig_timeout = 0;                  // сброс переменной ожидания импульса
         MDR_TIMER3->CNTRL |= 1;            // запуск Т3 --  
         break;             
