...
volatile bool     flg_cd_over = false;  // Флаг завершения отсчёта
...
void TIMER2_Init(void)
{
  ...
  // Настройка запросов на обработку прерываний
  MDR_TIMER2->IE = (1 << TIMER_IE_CNT_ARR_EVENT_IE_Pos);  // Прерывание при CNT = ARR
...
void Timer1_IRQHandler(void)
{
  ...
  // Установка флага завершения отсчёта
  flg_cd_over = true;
}
