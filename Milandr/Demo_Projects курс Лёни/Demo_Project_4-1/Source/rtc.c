...
void RTC_PrintTime(void)
{
  ...
  time_t unix_time = 0;           // Unix-время
  struct tm *broken_time = NULL;  // Компоненты времени

  // Получение Unix-времени
  RTC_WAIT_FOR_UPDATE;
  unix_time = MDR_BKP->RTC_CNT;
...
void RTC_SetTime(void)
{
  // Инициализация переменных
  time_t unix_time = 0;         // Unix-время
  struct tm broken_time = {0};  // Компоненты времени
