...
void TIMER_Init(void)
{
  ...
  MDR_TIMER1->PSG = MDR_TIMER2->PSG = 2000 - 1;

  // Период перезагрузки;
  // значение выбрано для периода в 1 секунду по формуле:
  // Tr = (PSG / CLK) * (ARR + 1)
  MDR_TIMER1->ARR = MDR_TIMER2->ARR = 39999;

  // Общая конфигурация
  MDR_TIMER1->CNTRL = MDR_TIMER2->CNTRL =
    (1 << TIMER_CNTRL_CNT_EN_Pos)      // Работа таймера (включён)
  ...
  MDR_TIMER1->DMA_RE = MDR_TIMER2->DMA_RE = TIMER_DMA_RE_CNT_ARR_EVENT_RE;  // Запрос при CNT = ARR
