...
void DMA_Init(void)
{
  ...
  DMA_CONTROL_DATA[8].DMA_CHANNEL_CFG = (1 << DMA_CHANNEL_CFG_CYCLE_CTRL_Pos)                     // Тип цикла (основной)
                                      | ((ADC_SAMPLE_SIZE - 1) << DMA_CHANNEL_CFG_N_MINUS_1_Pos)  // Кол-во передач в цикле (по размеру выборки)
  ...
  DMA_CONTROL_DATA[8].DMA_DST_DATA_END_PTR = (uint32_t)&adc_sample[ADC_SAMPLE_SIZE - 1];  // Приёмник
