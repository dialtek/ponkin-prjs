...
void PWM_SetPowerCycle(uint8_t power_cycle)
{
  ...
  MDR_TIMER3->CCR1 = MDR_TIMER3->ARR * power_cycle / 100;
  ...
  if (power_cycle >= 100 && flg_max_pwr == false) {
    ...
    MDR_TIMER3->CH1_CNTRL |= (5 << TIMER_CH_CNTRL_OCCM_Pos);
    flg_max_pwr = true;
  }
  ...
  // канал таймера переводится обратно в режим REF6
    ...
    MDR_TIMER3->CH1_CNTRL &= ~TIMER_CH_CNTRL_OCCM_Msk;
    MDR_TIMER3->CH1_CNTRL |= (6 << TIMER_CH_CNTRL_OCCM_Pos);
    flg_max_pwr = false;
  }
}
