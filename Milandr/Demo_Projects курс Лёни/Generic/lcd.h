...
#include <clk.h>       // Конфигурация системы тактирования
#include <font.h>      // Кодировка символов
...
// Макрос задержки (1 VAL = 1 мкс)
#define LCD_WAIT(TICK) for (volatile uint32_t i = 0;                                          \
                                              i < SystemCoreClock / (34 * 1000000) * (TICK);  \
                                              i++)

// Макрос подтяжки линии CS (PF2)
#define LCD_SPI_PULL_CS (MDR_PORTF->RXTX &= ~PORT_RXTX2_Msk)
