...
void DMA_Init(void)
{
  // Вычисление объёма передаваемых данных
  tx_size = strlen(tx_data);
  ...
  DMA_CONTROL_DATA[2].DMA_CHANNEL_CFG = (1 << DMA_CHANNEL_CFG_CYCLE_CTRL_Pos)             // Тип цикла (основной)
                                      | ((tx_size - 1) << DMA_CHANNEL_CFG_N_MINUS_1_Pos)  // Кол-во передач в цикле (по размеру массива)
  ...
  DMA_CONTROL_DATA[2].DMA_SRC_DATA_END_PTR = (uint32_t)&tx_data[tx_size - 1];  // Источник
  DMA_CONTROL_DATA[2].DMA_DST_DATA_END_PTR = (uint32_t)&MDR_UART2->DR;         // Приёмник
  ...
  DMA_CONTROL_DATA[3].DMA_CHANNEL_CFG = (1 << DMA_CHANNEL_CFG_CYCLE_CTRL_Pos)             // Тип цикла (основной)
                                      | ((RX_SIZE - 1) << DMA_CHANNEL_CFG_N_MINUS_1_Pos)  // Кол-во передач в цикле (по размеру массива)
  ...
  DMA_CONTROL_DATA[3].DMA_DST_DATA_END_PTR = (uint32_t)&rx_data[RX_SIZE - 1];  // Приёмник
...
void DMA_IRQHandler(void)
{
  ...
  if ((DMA_CONTROL_DATA[2].DMA_CHANNEL_CFG & DMA_CHANNEL_CFG_CYCLE_CTRL_Msk) == 0) {
    ...
    // Отключение запросов от UART к DMA на передачу данных
    MDR_UART2->DMACR &= ~UART_DMACR_TXDMAE;

    // Формирование нового цикла передач
    DMA_CONTROL_DATA[2].DMA_CHANNEL_CFG |= (1 << DMA_CHANNEL_CFG_CYCLE_CTRL_Pos)              // Тип цикла (основной)
                                         | ((tx_size - 1) << DMA_CHANNEL_CFG_N_MINUS_1_Pos);  // Кол-во передач в цикле
  ...
  if ((DMA_CONTROL_DATA[3].DMA_CHANNEL_CFG & DMA_CHANNEL_CFG_CYCLE_CTRL_Msk) == 0) {
    ...
    // Формирование нового цикла передач
    DMA_CONTROL_DATA[3].DMA_CHANNEL_CFG |= (1 << DMA_CHANNEL_CFG_CYCLE_CTRL_Pos)              // Тип цикла (основной)
                                         | ((RX_SIZE - 1) << DMA_CHANNEL_CFG_N_MINUS_1_Pos);  // Кол-во передач в цикле
