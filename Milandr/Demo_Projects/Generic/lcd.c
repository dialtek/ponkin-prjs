...
void LCD_Init(void)
{
  ...
  for (uint8_t i = 0; i < sizeof(lcd_cfg_table); i++) {
...
void LCD_WriteCmd(uint8_t cmd)
{
  ...
  // Синхронизирующая задержка (1 мкс)
  LCD_WAIT(1);
}
...
void LCD_PrintString(const char *str, uint8_t y, ...)
{
  ...
  char strf[LCD_SCREEN_WIDTH * 4];
  vsnprintf(strf, LCD_SCREEN_WIDTH * 4, str, arg_ptr);
  ...
  for (uint8_t i = 0; i < str_len && sym_pos < LCD_SCREEN_WIDTH; utf_sym = 0, sym_pos++) {
