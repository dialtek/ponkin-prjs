...
#define LCD_led_en MDR_PORTE->RXTX |= (1<<2) // РІРєР». РїРѕРґСЃРІРµС‚РєРё
#define LCD_led_dis MDR_PORTE->RXTX &= ~(1<<2) // РІС‹РєР». РїРѕРґСЃРІРµС‚РєРё
...
// -- LCD commands
  ...
  LCD_A0_OFF;
    ...
    static void LCD_wr_byte (uint8_t data) {
   ...
   static  void LCD_page_set (uint8_t page) {
    LCD_A0_OFF;
	  SPI1_Wr_Data(0xB0 | page);

  	}

    static void LCD_column_set (uint8_t column) {
    ...
    static void LCD_inverse(uint8_t state) {
  ...
  if(state)			
    ...
    static void LCD_inverse_OFF (void) {
...
// ------------------------------------ 
    static void LCD_set_cursor (uint8_t setPoint){
    ...
    static void LCD_wr_char (char symbol){
...
{ // вывод построчно 	
  LCD_wr_byte(LCD_5x7_font[char_code][i]);
    ...
    static void LCD_print_uni_logo (void) {
...
// -------------------------------------	
    static void LCD_print_num (int32_t number) {
  ...
  //--
    ...
    static void LCD_print_text (char* full_str){
    ...
    static void Milkites_welcome (void) {
