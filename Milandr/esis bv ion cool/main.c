/// блок вывода с модификация для ион-ионного охл.
...
//==============================================================================
  ...
  U16 pulse_delay = 500;        // длительсноть задержки перед новым импульсом вывода, кратно 10 мкс
  U16 pulse_count = 0;          // число получившихся импульсов за цикл
  ...
  U8 trig_in_progress = 0;      // статус наличия импульса запуска
...
//---CLK-----------------------------------------------------------------------------------------------------
  ...
  // прерывание 0,125 мкс
        ...
        MDR_TIMER2->ARR   = 7999;               // Основание счета, прерывание каждые 100 us
        MDR_TIMER2->IE    = 0x00000000;          // запрет прерывания по переполнению TIMER2  
  ...
  void timer2_set_mode(U16 per) {
     ...
     // предделитель = 1, Т2 в режиме отсчета участков импульса 
  if(per < 1)
        MDR_TIMER2->PSG   = 0;               // Предделитель частоты = 1, режим изм. длит. секций импульса вывода
  else
  {  // перевод Т2 в режим длит. прерываний с тиком 10 мкс
     MDR_TIMER2->PSG   = 799;                // Предделитель частоты = 800
     MDR_TIMER2->ARR   = per;                // Основание счета
     MDR_TIMER2->CNT   = 0;                  // сброс счетчика Т2 
     MDR_TIMER2->IE    = 0x00000002;          // разрешение прерывания по переполнению TIMER2 
     MDR_TIMER2->CNTRL |= 1;                  // Запуск Т2 
  }
}
...
//Режим работы каналов - ШИМ 
 ...
 MDR_TIMER3->STATUS= 0;                // сбрасываем флаги
  ...
  void extraction_stop(void) {
     /// завершение импульса вывода
     
     // остановка импульсов ШИМ и всего, что с ними связано
     ...
     {
       __no_operation();     // длительность полки
       __no_operation();
     }
     
     reset_integrator;                   // режим сброса вых.конденсатора
     
     // если есть импульс запуска
     if(trig_in_progress != 0)
     {
        
      timer2_set_mode(pulse_delay);      // задержка перед новым импульсом
     }
}

  void extraction_start(void){
  
  timer2_set_mode(0);       // уст. Т2 в режим отсчета врем. интервалов имульса
  frame = 0;                // уст. участка ШИМ = 1 участку     
  pulse_count++;            // инкремент счетчика импульсов

  
          // уст. скважности ШИМ в зависимости от режима работы
          if(mode == 0) 
          { /// импульс медленного вывода
...
{ 
    ...
    {  
       ...
       pulse_count = 0;                      // сброс счетчика импульсов за период
       ...
       if((frame_time[0] > 1) || (mode > 0)) // если t медл вывода > 1 или выбран быстр. вывод
          extraction_start();                // старт секций импульса    
    }
...
{
      // если есть прерывание и предделитель Т2 > 0, значит новый импульс вывода 
      // в пределах импульса запуска -> инициализация его и старт
      if(MDR_TIMER2->PSG > 0) extraction_start();
      
      // есть прерывание и предделитель = 0 -> формирование участков импульса вывода
      else
      {
       ...
       {
        case 0:
          if( (frame_time[1] < 1) || (mode == 1) ) extraction_stop(); // если t уч.2 < 1 и не быстр. вывод, то метод завершения 
          else 
          {
...
//---------------------------------------------    
    ...
    if(answer == modbus_rhr_cmd) // ответ при команде чтения регистров
    {
         ...
         holding_register[8] = end_delay;               // Длительность полки, пауза для устранения кросс токов закрытия ключа сброса интегратора                                           // в микросекундах
         holding_register[9] = pulse_delay*10;          // задержка перед новым импульсом, кратна 10 мкс 
         ...
         holding_register[16] = pulse_count;            // число импульсов за период
         holding_register[17] = trig_in_progress;       // статус наличия импульса запуска
    ...
    if(answer == modbus_wsr_cmd) // ответ при команде записи в регистр
    {
	 ...
	 //--------------------------------------------------------------------
         ...
         if(holding_register[8] < 10)    holding_register[8] = 10;  // мин. Т полки
         if(holding_register[8] > 2000)  holding_register[8] = 2000;// макс.Т полки
         end_delay = holding_register[8];            // Длительность полки 
         
         if(holding_register[9] < 100)   holding_register[9] = 100;  // мин. задержка
         if(holding_register[9] > 65000) holding_register[9] = 65000;// макс. задержка
         
         pulse_delay = holding_register[9]/10;       // задержка перед новым импульсом, кратна 10 мкс 
