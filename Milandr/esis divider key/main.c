...
/// устройство - блок коммутации резистора секции 7 структуры дрейфа
...
// используется нижний драйвер IR2184, поэтому ключи работают в обратной логике
...
#define dev_id 42               // modbus id текущего устройства                        <<<<<<<<<<=========================== ID
#define firmware_ver 1          // версия прошивки текущего устройства
#define device_family 4         // код семейства устройств: 1 - PBF modules, 2 - extraction modules, 3 - dc hv modules, 4 - divider keys
...
//==============================================================================
  ...
  U16 resistor_sw_state = 0;      // состояние транзисторных ключей: 0 - 510R, 1 - 1k, 2 - 2k
...
//---CLK-----------------------------------------------------------------------------------------------------
  ...
  //---------------------------------------------------------------------------//
  MDR_PORTB->OE = 0xFFFF;               // порт B на выход, PB0 - S1, PB1 - S2, PB2 - S3
  ...
  //---------------------------------------------------------------------------//
  MDR_PORTD->OE = 0xffd3;               // порт D на вход, PD2 - ADC_CH2, PD3 - ADC_CH3, PD5 - ADC_CH5
  MDR_PORTD->FUNC = 0x0000;             // функция - порт 
  MDR_PORTD->ANALOG  = 0xffd3;          // режим потра - цифровой, но PD2 - ADC_CH2, PD3 - ADC_CH3, PD5 - ADC_CH5
...
{  
 U32 led_delay_cnt = 0;
 U32 section_2_volt = 0;        // измеренной значение напряж. с делителя секции 2
 U32 section_7_volt = 0;        // измеренной значение напряж. с делителя секции 7
 U32 section_24_volt = 0;       // измеренной значение напряж. с делителя секции 24
         ...
         case detect_dev_id:              // ждем обращения к устройтсву
           switch(rx_byte)
           {
             case dev_id:                 // получена команда обращения по текущему id устройтсва
               Status_LED_OFF;            // индикация получения посылки
...
//---------------------------------------------   
    ...
    if(answer == modbus_rhr_cmd) // ответ при команде чтения регистров
    {
         ...
         holding_register[0] = resistor_sw_state;    // состояние транзисторных ключей: 0 - 510R, 1 - 1k, 2 - 2k
         holding_register[3] = (U16)section_2_volt;  // напряж. с делителя секции 2
         holding_register[4] = (U16)section_7_volt;  // напряж. с делителя секции 7
         holding_register[5] = (U16)section_24_volt; // напряж. с делителя секции 24
    ...
    if(answer == modbus_wsr_cmd) // ответ при команде записи в регистр
    {
	 ...
	 //--------------------------------------------------------------------
         ...
         // состояние транзисторных ключей: 0 - 510R, 1 - 1k, 2 - 2k
...
//--------------------------------------------------------------------  
     // измерения напряжения с делителей
     ...
     // преобразование в волты измеренных отсчетов с секции 24
     section_24_volt     = (U32)(ADC_code*0.8056640625);//ADC_Vref*1000.0)/4096.0);
     
     MCU_ADC_init();                             // сброс предыдущих настроек
     MCU_ADC_set_ch(3);                          // установка канала АЦП 
     // чтение отсчетов АЦП 
     ADC_code  = (S32)MCU_ADC_Rd_average(MCU_ADC_aver_param); 
     // преобразование в волты измеренных отсчетов с секции 7
     ...
     // чтение отсчетов АЦП 
     ADC_code  = (S32)MCU_ADC_Rd_average(MCU_ADC_aver_param); 
     // преобразование в волты измеренных отсчетов с секции 2
