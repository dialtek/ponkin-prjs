...
#define dev_id 100              // modbus id текущего устройства
...
#define get_reg_addr_msb   12      // код 2 состояния кон.автомата
#define get_reg_addr_lsb   13      // код 3 состояния кон.автомата
...
#define send_answer        18      // код 8 состояния кон.автомата
 ...
 //функция обработки прерывания irq UART
{
  /// прерывание возникает при поступлении 8 байт в буфер FIFO приемника UART !!
         ...
         switch(Rx_data[0])      // нулевой байт - заголовок команды  
         {
              ...
              //--
              default: 
                Uart_text_send("error"); // неизвестная команда
                Uart_CR_LF_send();   
          }// switch
...
{     
 ...
 U8 ans_flag = 0;  // флаг необходимости ответа на команду ПК
 ...
 U8  crc_buf[16];            // буфер для хранения байтов для расчета CRC16
 ...
 NVIC_EnableIRQ(Timer1_IRQn); // Разрешение прерывания для T1 - MODBUS
 ...
 //Status_LED_ON;  // включаем статусный светодиод
 ...
 while(1)
  {
    ...
    if(need2setDAC) // действия при обновлении кода DAC2 с ПК
    {
      ...
      else
      {
        ...
        Uart_text_send("DAC=");         // ответ пользователю - подтвержение 
    ...
    {                                     
       ...
       //=====                          
         case get_cmd_header:             // анализ команды modbus
           switch (rx_byte)
           { // переход в сост. получения адреса регистра modbus
             case modbus_rhr_cmd: rd_state = get_reg_addr_msb;  break;          
             case modbus_wsr_cmd: rd_state = get_reg_addr_msb;  break; 
             // возврат в сост. анализ id устройства в случае ошибки команды  
             default: rd_state = detect_dev_id;
           }
         break;
        //=====
         case get_reg_addr_msb:              // получение ст. байта адреса регистра
             temp_buf = (U16)rx_byte;        // сохр. ст. байт
             temp_buf = temp_buf << 8;
             rd_state = get_reg_addr_lsb;    // переход в сост. чтения мл. байта
         break;
         //=====
         case get_reg_addr_lsb:              // получение мл. байта адреса регистра
             temp_buf |= (U16)rx_byte;       // сохр. мл. байт
            // имеется такой регистр?
             if(temp_buf == volt_lvl_reg)
             {  // да, переход в сост. чтения числа регистров для чтения
                modbus_reg_addr = temp_buf;    // сохр. начального адреса регистров для чтения
                rd_state = get_reg_cnt_msb;    // переход в сост. чтения мл. байта
             }
             // не имеется, возврат в сост. чтения id устройства
             else  rd_state = detect_dev_id;
         break;
         
         case get_reg_cnt_msb:              // получение ст. байта кол-ва регистров для чтения
             temp_buf = (U16)rx_byte;       // сохр. ст. байт
             temp_buf = temp_buf << 8;
             rd_state = get_reg_cnt_lsb;
         break;
         //=====
         case get_reg_cnt_lsb:              // получение мл. байта кол-ва регистров для чтения
             temp_buf |= (U16)rx_byte;      // сохр. мл. байт
             // адекватное число?
             if(temp_buf < max_regs_cnt)
             {  // да
                regs2read = temp_buf;      // сохранение числа регистров для чтения
                rd_state = calc_crc1;//detect_dev_id;       // переход в состояние расчета CRC16 
             }
             // нет, возврат в сост. чтения id устройства
             else   rd_state = detect_dev_id;  
          break;  
         //=====
         case calc_crc1:                     // получение ст. байта CRC
             temp_buf = (U16)rx_byte;      // сохр. мл. байт
             temp_buf = temp_buf << 8;
             rd_state = calc_crc2; 
         break; 
         //===== 
         case calc_crc2:                    // получение мл. байта CRC
             temp_buf |= (U16)rx_byte;      // сохр. мл. байт
...
//             Uart_num_send(temp_buf);
       ...
       if(ans_flag)
       {
          ...
          crc_buf[0] = dev_id;
          crc_buf[1] = modbus_rhr_cmd;
          crc_buf[2] = 2; // кол-во байт данных 
          ...
          Uart_send_hex(0x02); // кол-во байт данных 
