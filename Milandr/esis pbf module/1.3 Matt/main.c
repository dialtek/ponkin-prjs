...
/// устройство - блок формирования потенциальных барьеров структуры дрейфа КРИОН

// ВЕРСИЯ ПРОШИВКИ ДЛЯ СТЕНДА ВНЕШНЕЙ ИНЖЕКЦИИ
...
//==============================================================================
  ...
  S32 DAC_code_second = 0;         // кода ЦАП - вторая часть импульса
  ...
  U16 V2set_2 = 0;                 // для хранения уставки с ПК, вторая часть сложного импульса
  ...
  float kp = 3.0;        // коэффициент П-звена регулятора
  float ki = 0.6;        // коэффициент I-звена регулятора
...
//---CLK-----------------------------------------------------------------------------------------------------
  ...
  // настройка таймера для прерывания прерывание 10 мкс + пер и зад. фронт
    
  MDR_RST_CLK->TIM_CLOCK  |= (1 << 24);      // разрешение тактирования Timer1, TIM1_CLK = 80 МГц												
  ...
  MDR_TIMER1->PSG   = 799;                         	  // Предделитель частоты тактирования Т1 - 1
  MDR_TIMER1->ARR   = 9;                                  // Основание счета, таймер тикает до  
  MDR_TIMER1->IE    = (1<<5) | (1<<13);                   // разрешения прерывания по переднему и заднему фронту 1 канала, выкл. прерывание по совпадения CNT и ARR
 ...
 MDR_TIMER3->ARR   = 199;                	 // основание счета = 200, 200 * 5 мс = 1000 мс
...
{       
   ...
   if(int_flag & (1<<1))  // прерывание по переполнению таймера 10 us
   { 
     frame_cnt++;
     
     if(frame_cnt >= frame_set) 
     {
       DAC_code = DAC_code_second;
     }
   }
   ...
   {  
     MDR_TIMER1->IE &= ~(1<<1);
     DAC_code = DAC_code_ramp; // сброс уставки на изначальный
     frame_cnt = 0;
   }
...
{  // защита от перегрузки по току
    ...
    switch (overcurr_state)
    {
      case 0: 
        Error_LED_OFF;
        if(overcurrent_count > 4) overcurrent_bit = 1; // уст. флаг перегрузки по току 
...
/*========================================================================== */
/*
  // расчет скользящего среднего обратной связи ПИД-регулятора
  S32 pid_moving_aver(S32 data)
  {
    S32 aver;
    
    if (pid_m >= pid_n) // буфер усреднения заполнился?
        // да
      pid_sum = pid_sum - pid_aver_buf[pid_k] + data; // удаление предыдущего эл, добавление нового
    else
    {   // нет
       pid_sum += data; // копим сумму
       pid_m++;
    }
      pid_aver_buf[pid_k] = data; // сохр. нового значения для след. итерации
      aver = pid_sum / pid_m;     // расчет среднего значения 
      pid_k++;                // инкремент индекса массива
      if (pid_k >= pid_n) pid_k = 0;  // сброс индекса если переполнился
  
      return aver;
  }
  
  // расчет скользящего среднего измерений тока нагрузки
  U32 iload_moving_aver(U32 data)
  {
    U32 aver; 
    
    if (iload_m >= iload_n) // буфер усреднения заполнился?
        // да
      iload_sum = iload_sum -iload_aver_buf[iload_k] + data; // удаление предыдущего эл, добавление нового
    else
    {   // нет
       iload_sum += data; // копим сумму
       iload_m++;
    }
      iload_aver_buf[iload_k] = data; // сохр. нового значения для след. итерации
      aver = iload_sum / iload_m;     // расчет среднего значения 
      iload_k++;                // инкремент индекса массива
      if (iload_k >= iload_n) iload_k = 0;  // сброс индекса если переполнился
...
//---------------------------------------------    
    ...
    if(answer == modbus_rhr_cmd)          // ответ при команде чтения регистров
    {
         ...
         holding_register[8] = (U16)ramp_in_progress;           // статус бит плавной уст. заданного выс. напряженения
         ...
         holding_register[15] = (U16)device_family;             // семейство устройства 1 - PBF modules
         
         holding_register[16] = (U16) frame_set;
         holding_register[17] = V2set_2;
    ...
    if(answer == modbus_wsr_cmd)          // ответ при команде записи в регистр
    {
	 ...
	 //--------------------------------------------------------------------
         ...
         kp = (float)holding_register[1];
         ki = (float)(holding_register[2]/10.0);
         kd = (float)holding_register[3];
         
         if(holding_register[0] > 2600) holding_register[0] = 2600; 
         ...
         frame_set = holding_register[16];  
         V2set_2 = holding_register[17];
         
         DAC_code_second = (S32)(V2set_2 / 0.805664); // преобразование вольт в отсчета ЦАП, вторая часть импульса
...
//======================================     
     ...
     ADC_V     = (U32)(ADC_code*0.8056640625);//ADC_Vref*1000.0)/4096.0);
//======================================      
     // ПИД-регулятор высокого напряжения
     if(HV_EN_bit & (!overcurrent_bit))
     {
     //--------- 
     ...
     p = (kp*error);                             // П-звено регулятора
     i = (i + error*ki);                         // I-звено регулятора
     d = ((prev_error - error)*kd);              // I-звено регулятора
     prev_error = error;
     
     if(i > 5080) i = 5080;                      // ограничение интегрального звена регулятора
     if(i < -5080) i = -5080;

     pid = (S32)(p + i + d);                     // суммирование членов ПИД-регулятора

     if(pid > 5080) pid = 5080;                  // ограничение выходного кода ЦАП
     if(pid < 0) pid = 0;
     // уст. кода регулятора в ЦАП, перевод из Вольт в отсчеты
     
     MDR_DAC->DAC2_DATA = (U32)(pid*0.805664);
    }
    else MDR_DAC->DAC2_DATA = 0;
     
//======================================      
     // измерения тока нагрузки
     MCU_ADC_init();                             // сброс предыдущих настроек
