...
#define TRIG_LED_ON  	MDR_PORTE->RXTX |=  1<<3 
#define TRIG_LED_OFF  	MDR_PORTE->RXTX &= ~(1<<3) 
...
//==============================================================================
  ...
  U8 trig_in_progress = 0;         // статус-бит импульса запуска 
...
//---CLK-----------------------------------------------------------------------------------------------------
  ...
  //                      режим CAPTURE     отрицательный фронт
  MDR_TIMER1->CH1_CNTRL = (1<<15)               | (1<<4);              
  //                      CCR1 разрешен     положительный фронт 
  MDR_TIMER1->CH1_CNTRL2 =(1<<2)                | (0<<1);	        
  ...
  // 5 - зад. фронт 
  // 13 - пер. фронт
  MDR_TIMER1->IE    = (1<<5) | (1<<13);                   // разрешения прерывания по переднему и заднему фронту 1 канала, вкл. прерывание по совпадения CNT и ARR
...
{       
   ...
   if(int_flag & (1<<5))    //  \___ флаг прерывания по заднему фронту
   {  
    trig_in_progress = 0;                  // сброс статус-бита импульса запуска 
   }     
   
   if(int_flag & (1<<13))  //  ___/ флаг прерывания по переднему фронту
   {  

           TRIG_LED_ON;                    // вкл. светодиод запуска
       trig_led_delay_cnt = 0;             // сброс счетчика вкл. светодиода
       trig_in_progress = 1;               // уст. статус-бит импульса запуска 
   }
...
//======================================      
     ...
     I_load_buf[I_lim_cnt] = I_load_temp;        // сохр. в буфер ск. сред.
     
     I_lim_cnt++;                                // инк. счетчика буфера ск. сред.
     if(I_lim_cnt > I_aver_size-1) I_lim_cnt = 0;// сброс при переполнении счетчика

     I_load_temp = 0;
     for(U8 i = 0; i < I_aver_size; i++)        
      I_load_temp += I_load_buf[i];              // суммирование буфера ск. сред.
     
     I_load = I_load_temp/(I_aver_size); // расчет скользящего среднего знач. тока
//======================================     
...
//====================================== 
    ...
    if(trig_in_progress)
      trig_led_delay_cnt = 0;   // сброс счетчика задержки светодиода запуска
    else
      trig_led_delay_cnt++;     // инкремент счетчика задержки светодиода запуска
