...
/* USER FUNCS */
  ...
  if(cmd_type == MODBUS_RHR_CMD)
   {   
        ...
        // ADCs
        for(U8 i = 1; i <= 64; i++) 
          holding_register[i] = InputConnPin[i]; // сохр. 64 каналов в регистры
        
        holding_register[66] = meas_mode;    // режим измерений 0 - непрерывные, 1 - импульсный по запуск   
        holding_register[67] = int_time;     // время интегирования
        holding_register[68] = ADC_scale;    // шкала измерений'0' - 3pC, '1' - 6pC, '2' - 12pC
   ...
   {  // анализ регистра записи 
      switch(addr_buf_1) {  
        case 66: // режим измерений
         meas_mode = holding_register[66]; // 0 - непрерывные, 1 - импульсный по запуску
        break;
        //---------
        case 67: // t интегрирования
         if(holding_register[67] > 65000)  
           holding_register[67] = 65000; // 65 ms max
         else if(holding_register[67] < 10)
           holding_register[67] = 10;    // 10 us min
         
         int_time = holding_register[67]; // сохр. t интегрирования
        break;
       //---------
        case 68: // шкала измерений
         if(holding_register[68] > 2)  
           holding_register[68] = 2;   // 12 pC max
         ADC_scale = holding_register[68]; // шкала измерений'0' - 3pC, '1' - 6pC, '2' - 12pC
         ADC_change_scale();   
        break;
        //---------
       default: break; } 
