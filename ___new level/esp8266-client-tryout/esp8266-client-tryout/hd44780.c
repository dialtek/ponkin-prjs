...
/**
 * Changes by Camil Staps <info@camilstaps.nl>
 *
 * I changed DATA_SHIFT into LCD_DATA_SHIFT for compatibility.
 * Similar for DATA_MASK into LCD_DATA_MASK.
 */
...
static void _send_nibble(unsigned char data) {
    data <<= LCD_DATA_SHIFT;          // shift the data as required
    LCD_DATA &= ~LCD_DATA_MASK;       // clear old data bits
    LCD_DATA |= LCD_DATA_MASK & data; // put in new data bits
