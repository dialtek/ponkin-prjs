...
namespace BUV_ctrl
{
   ...
   /// </summary>
    public class Source
    {
            ...
            /// <summary>
            /// Запрос на получение напряжения источника
            /// </summary>
            /// <returns></returns>
            public string GetVoltage(IPEndPoint IpAndPort)
            {
                string Data = "-";
            ...
            /// <summary>
            /// Запрос на получение тока источника
            /// </summary>
            /// <returns></returns>
            public string GetCurrent(IPEndPoint IpAndPort)
            {
                string Data = "-";

                if (Connect2server(IpAndPort))
                {   // читаем напряжение источника
                    Data = SrcCmd.GetCurrent(ns);
                }
                DisconnectServer();
