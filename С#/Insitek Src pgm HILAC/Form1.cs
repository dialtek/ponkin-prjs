...
namespace Insitek_src
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            Src[0].Interval = 300;
            Src[0].IpAdress = "192.168.127.254";//"172.16.21.125";//"172.16.21.117";    //IPadrList[NumberOfSources];
            Src[0].Port = 4001;                     //Convert.ToInt32(PortList[NumberOfSources]);
            Src[0].ID = 15;                         //NumberOfSources;
            Src[0].Init();
        ...
        void ListView1_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            ...
            try
            {
                if (ListView1.SelectedIndices.Count > 0)
                {
                    ...
                    IPV60_VoltUpDown.Value = Convert.ToDecimal(U2set_str); // записываем в UpDown

                    if (SrcModbusAddr == 15) IPV60_VoltUpDown.Maximum = 60;
                    else if (SrcModbusAddr == 247) IPV60_VoltUpDown.Maximum = 120;
                }
            }
            catch (Exception)
            {
        ...
        {
            /// Вызов события установки напряжения на ист. ипв 60
            ///  
           
            string buf;
            buf = IPV60_VoltUpDown.Value.ToString();
            U2set_str = buf;
            buf = buf.Replace(";", "");
            buf = buf.Replace(",", "");
        ...
        private void Update_timer_Tick(object sender, EventArgs e)
        {
            ...
            {
                IPV60_ChargeOn_B.Enabled = false;
                IPV60_ChargeOff_B.Enabled = true;
                OutState_1.BackColor = Color.LimeGreen;
            }
            else
            {
                IPV60_ChargeOn_B.Enabled = true;
                IPV60_ChargeOff_B.Enabled = false;
                OutState_1.BackColor = Color.LightGray;
            }

            if (SrcOutStatus[1][2] == '1') // состояние выхода
            {
                IPV120_ChargeOn_B.Enabled = false;
                IPV120_ChargeOff_B.Enabled = true;
                OutState_2.BackColor = Color.LimeGreen;
            }
            else
            {
                IPV120_ChargeOn_B.Enabled = true;
                IPV120_ChargeOff_B.Enabled = false;
                OutState_2.BackColor = Color.LightGray; 
            }
