...
namespace LXM_ctrl
{
    enum cmd_type
    {
        ...
        wmr = (byte)0x10    // modbus Write Multiple Registers command 
    }
    ...
    class modbus_rtu_helper
    {
        ...
        public byte[] build_wsr_cmd(byte device_id, ushort reg_addr, ushort data2wr)
        {
            ...
            CmdBuf[0] = device_id;     // id
            CmdBuf[1] = (byte)cmd_type.wsr;      // WriteSingleRegister cmd
            CmdBuf[2] = addr_msb;                // адрес регистра для записи ст. байт
            CmdBuf[3] = addr_lsb;      // адрес регистра для записи мл. байт 
            CmdBuf[4] = data2wr_msb;   // ст.байт данных
            CmdBuf[5] = data2wr_lsb;   // мл. байт данных

            crc16 = calc_crc(CmdBuf, 6);// crc текущей команды подсчет и сохранение 
        ...
        /// <summary>
        /// Формирование посылки байт для записи нескольких регистров начиная с начального адреса
        /// </summary>
        /// <param name="device_id"></param>
        /// <param name="reg_addr"></param>
        /// <param name="data2wr"></param>
        /// <returns></returns>
        public byte[] build_wmr_cmd(byte device_id, ushort reg_addr, byte[] data2wr)
        {
            ushort crc16 = 0x0000;
            byte crc16_msb = 0x00;
            byte crc16_lsb = 0x00;

            UInt16 reg_cnt = (byte)data2wr.Count();           // число байт в массиве для отправки
            int count = reg_cnt - 1;

            // 8 - id,cmd,bytes count, CRC...etc
            byte[] CmdBuf = new byte[reg_cnt+9]; // буфер команды для отправки
            reg_cnt /= 2;  // приводим к регисрам, 2 байта = 1 рег.

            byte addr_msb = (byte)(reg_addr >> 8); // ст. байт адреса
            reg_addr = (byte)(reg_addr & 0x00ff);
            byte addr_lsb = (byte)(reg_addr);      // мл. байт адреса

            byte count_msb = (byte)(reg_cnt >> 8); // ст. байт адреса
            reg_cnt = (byte)(reg_cnt & 0x00ff);
            byte count_lsb = (byte)(reg_cnt);      // мл. байт адреса

            CmdBuf[0] = device_id;               // id
            CmdBuf[1] = (byte)cmd_type.wmr;      // WriteMultipleRegisters cmd
            CmdBuf[2] = addr_msb;                // адрес регистра для записи ст. байт
            CmdBuf[3] = addr_lsb;                // адрес регистра для записи мл. байт 
            CmdBuf[4] = count_msb;               // число регистров для записи ст. байт
            CmdBuf[5] = count_lsb;               // число регистров для записи ст. байт
            CmdBuf[6] = (byte)(reg_cnt * 2);     // число байт в поле данных

            for (int i = 0; i < reg_cnt*2; i++)
            {   // запись данных
                CmdBuf[i + 7] = data2wr[count];
                //CmdBuf[count + 8] = data2wr[];
                count--;
            }

            crc16 = calc_crc(CmdBuf, 7 + reg_cnt * 2);    // crc текущей команды подсчет и сохранение 

            crc16_msb = (byte)(crc16 >> 8); // ст байт контрольной суммы
            crc16 = (byte)(crc16 & 0x00ff);
            crc16_lsb = (byte)(crc16);

            CmdBuf[7 + reg_cnt * 2] = crc16_lsb;
            CmdBuf[8 + reg_cnt * 2] = crc16_msb;
        ...
        private List<string> parse_read_answer(byte[] modbus_array)
        {
            ...
            for (int i = 0; i < count / 2; i++)
            {
                ...
                word = (UInt16)((msb << 8) + lsb); // объединение байт
        ...
        private string parse_write_multReg_answer(byte[] modbus_array)
        {

            byte msb = modbus_array[2]; // 1st reg addr msb
            byte lsb = modbus_array[3]; // 1st reg addr lsb
        ...
        /// <summary>
        /// запись данных в регистр modbus с подтверждением ответа
        /// </summary>
        /// <param name="ns"></param>
        ...
        private List<string> write_mult_regs(NetworkStream ns, byte[] write_array)
        {
            byte[] dev_wr_answer = new byte[7];             // буфер для чтения ответа
            List<string> return_list = new List<string>();  // список для сохр. ответа на команду записи
            string reg_data = "";
            try
            {
                ns.Write(write_array, 0, write_array.Count());                      // отсылка команды записи регистра                         
                //for (int i = 0; i < 7; i++) dev_wr_answer[i] = (byte)ns.ReadByte(); // чтение

                //if (dev_wr_answer.Count() == 7)                                     // сравнение массивов элемент к элементу
                //{
                //    reg_data = parse_write_multReg_answer(dev_wr_answer);           // сохр. считаное значение регистра если все ок     
                //}
                //else reg_data = "crc error";    // ошибка контрольной суммы
            }
            catch { reg_data = "r/w error"; }   // ошибка чтения или записи
        ...
        private List<string> read_regs(NetworkStream ns, byte[] read_array)
        {
            ...
            try
            {
                ...
                crc_rd = (UInt16)((dev_rd_answer[crc_msb_index] << 8) + dev_rd_answer[crc_lsb_index]);      // сохр. считанной контрольной суммы
        ...
        /// <param name="read_array"></param>
        /// <returns></returns>
        public KeyValuePair<Dictionary<string, UInt16>, List<string>> do_cmd(NetworkStream ns, byte[] read_array)
        {
            // словарь контейнера возврата - информация о передаваемых данных
            Dictionary<string, UInt16> resp_info = new Dictionary<string, UInt16>();
            ...
            switch (read_array[1]) // анализ команды
            {
                ...
                case (byte)cmd_type.wmr: response = write_mult_regs(ns, read_array); break; // ответ при групповой записи в регистры

                default: response.Add("unknown command"); break;
            }


            resp_info.Add("id", (UInt16)read_array[0]);      // запись id отвечающего устройства     
            resp_info.Add("cmd", (UInt16)read_array[1]);      // запись команды, на которую отвечает устройтво 
            resp_info.Add("reg", (UInt16)((read_array[2] << 8)// чтение - начальный адрес регистра чтения, запись - регистр записи   
                                        + read_array[3]));
            if (read_array[1] == (byte)cmd_type.rhr)
            {   // если чтение - вставляем содержание регистра числа регистров
                resp_info.Add("count", (UInt16)((read_array[4] << 8) + read_array[5]));
            }
            else resp_info.Add("count", 1);  // если запись, то читается 1 регистр 

            KeyValuePair<Dictionary<string, UInt16>, List<string>> modbus_container
                                 = new KeyValuePair<Dictionary<string, UInt16>, List<string>>(resp_info, response);
